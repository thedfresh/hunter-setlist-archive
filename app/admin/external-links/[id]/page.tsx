    ...
    async function fetchLink() {
      ...
      try {
        ...
        if (res.ok && data.association) {
          setForm({
            url: data.association.link.url,
            title: data.association.link.title || "",
            description: data.association.link.description || "",
            entityType: data.association.entityType,
            entityId: String(data.association.entityId),
            linkType: data.association.linkType,
            isPublic: !!data.association.isPublic,
            linkId: String(data.association.linkId),
          });
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="max-w-xl mx-auto bg-white rounded-xl shadow-lg p-8">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Edit External Link</h1>
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={saving} className="w-full bg-blue-600 text-white font-semibold py-2 px-4 rounded-md shadow hover:bg-blue-700 transition">
              {saving ? "Saving..." : "Save Changes"}
            </button>
            <button type="button" onClick={handleDelete} disabled={saving} className="w-full mt-2 bg-red-600 text-white font-semibold py-2 px-4 rounded-md shadow hover:bg-red-700 transition">
              Delete Link
            </button>
            {success && <div className="mt-4 text-green-600 text-center">Link updated successfully!</div>}
          </form>
        )}
      </div>
    </div>
