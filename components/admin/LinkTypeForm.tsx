...
export default function LinkTypeForm({ linkTypeId, onSuccess, onCancel }: LinkTypeFormProps) {
    ...
    useEffect(() => {
        if (linkTypeId > 0) {
            setLoading(true);
            fetch(`/api/admin/link-types/${linkTypeId}`)
                .then(res => res.json())
                .then(data => {
                    setName(data.name || '');
                    setDescription(data.description || '');
                })
                .catch(() => setError('Failed to load link type'))
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const res = await fetch(
                linkTypeId > 0 ? `/api/admin/link-types/${linkTypeId}` : '/api/admin/link-types',
                {
                    method: linkTypeId > 0 ? 'PUT' : 'POST',
